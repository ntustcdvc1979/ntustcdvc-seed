    ...
    <div className="space-y-10">
      ...
      {category === '修' && (
        <Quadrant title="心靈平靜 (修)" motto="修道越修越歡喜" icon="🧘" colorType="green">
          ...
          <SkillCircle name="初一十五獻供" count={userData?.stats?.初一十五獻供} onClick={() => incrementSkill('初一十五獻供')} onDecrement={() => decrementSkill('初一十五獻供')} img="🍎" colorType="green" />
          ...
          <SkillCircle name="壇務工作" count={userData?.stats?.壇務工作} onClick={() => incrementSkill('壇務工作')} onDecrement={() => decrementSkill('壇務工作')} img="🕯️" colorType="green" />
          <SkillCircle name="法會實務" count={userData?.stats?.法會實務} onClick={() => incrementSkill('法會實務')} onDecrement={() => decrementSkill('法會實務')} img="🏗️" colorType="green" />
      ...
      {category === '講' && (
        <Quadrant title="正向溝通 (講)" motto="講道越講入性理" icon="🗣️" colorType="yellow">
          <SkillCircle name="活動帶朋友" count={userData?.stats?.活動帶朋友} onClick={() => incrementSkill('活動帶朋友')} onDecrement={() => decrementSkill('活動帶朋友')} img="🤝" colorType="yellow" />
      ...
      {category === '辦' && (
        <Quadrant title="行動服務 (辦)" motto="辦道越辦越契機" icon="👼" colorType="yellow">
          ...
          <SkillCircle name="策畫結緣活動" count={userData?.stats?.策畫結緣活動} onClick={() => incrementSkill('策畫結緣活動')} onDecrement={() => decrementSkill('策畫結緣活動')} img="🎃" colorType="yellow" />
        </Quadrant>
      )}
    </div>
  );
